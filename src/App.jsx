...
if (hasValidConfig) {
    ...
    console.warn("No Firebase API Key found! Game will run in offline mode and progress won't be saved.");
}
...
function TycoonGame({ onExit }) {
    ...
    useEffect(() => {
        ...
        const resetGame = async () => {
            if (window.confirm('Wipe all Tycoon data? This cannot be undone.')) {
                ...
                // Reset to initial game state regardless of online/offline
                setGameState({
                    money: 50,
                    filament: 1000,
                    inventory: {},
                ...
                playerRef.current = { x: 900, y: 1200, angle: 0 };
            }
        };
    ...
    useEffect(() => {
        ...
        const gameLoop = (time) => {
            ...
            pads.forEach(pad => {
                ...
                if (px >= pad.x - pad.w / 2 && px <= pad.x + pad.w / 2 && py >= pad.y - pad.h / 2 && py <= pad.y + pad.h / 2) {
                    const
    ...
    // Cloud Auto-Save
    useEffect(() => {
        if (!user || !isLoaded || !hasValidConfig) return;
        const saveInterval = setInterval(() => {
            const saveRef = doc(db, 'artifacts', appId, 'users', user.uid, 'saveData', 'tycoonSave');
            setDoc(saveRef, stateRef.current).catch(console.error);
        }, 5000);
        return () => clearInterval(saveInterval);
    }, [user, isLoaded, hasValidConfig]);
    ...
    const getActivePads = (state) => {
            ...
            pads.push({
                ...
                action: (s) => {
                    ...
                    setGameState(prev => {
                        ...
                        return collected ? {
                            ...prev, inventory: newInv, stats: {
                                ...prev.stats, totalPrinted: prev.stats.totalPrinted +
                                    count
            ...
            pads.push({
                ...
                action: (s) => {
                    ...
                    setGameState(prev => {
                        ...
                        return sold ? {
                            ...prev, money: prev.money + revenue, inventory: newInv, stats: {
                                ...prev.stats, totalEarned:
                                    prev.stats.totalEarned + revenue
                            }
                        } : prev;
                    });
                    return sold;
        ...
        pads.push({
            ...
            action: (s) => {
                if (s.money < FILAMENT_PRICE) return false; setGameState(prev => ({
                    ...prev, money: prev.money - FILAMENT_PRICE,
        ...
        if (state.upgrades.bulkSupplier) {
            pads.push({
                ...
                action: (s) => {
                    if (s.money < BULK_FILAMENT_PRICE) return false; setGameState(prev => ({
        ...
        if (currentPrinterCount < TYCOON_PROGRESSION.length) {
            const
            nextPrinter = TYCOON_PROGRESSION[currentPrinterCount]; const slot = PRINTER_SLOTS[currentPrinterCount];
            pads.push({
                id: `buy_printer_${currentPrinterCount}`, x: slot.x, y: slot.y + 120, w: 120, h: 80,
        ...
        if (!state.upgrades.bulkSupplier && state.printers.length >= 2) {
            pads.push({
                ...
                text: 'UPGRADE:\nBULK SUPPLY\n$200',
                action: (s) => {
                    if (s.money < 200) return false; setGameState(prev => ({
                        ...prev, money: prev.money - 200,
        ...
        if (!state.upgrades.autoManager && state.printers.length >= 3) {
            pads.push({
                ...
                text: 'UPGRADE:\nAUTO-MGR\n$500',
                action: (s) => {
                    if (s.money < 500) return false; setGameState(prev => ({
                        ...prev, money: prev.money - 500,
        ...
        if (!state.upgrades.autoSell && state.printers.length >= 4) {
            pads.push({
                ...
                text: 'UPGRADE:\nAUTO-SELL\n$1500',
                action: (s) => {
                    if (s.money < 1500) return false; setGameState(prev => ({
                        ...prev, money: prev.money -
        ...
        state.printers.forEach((p, idx) => {
            ...
            pads.push({
                ...
                action: (s) => {
                    ...
                    setGameState(prev => ({
                        ...prev,
                        printers: prev.printers.map(printer => printer.id === p.id ? {
                            ...printer, modelId:
                                nextModel
                        } : printer)
                    }));
    ...
    return (
        <div
            ...
            <div className="absolute top-0 left-0 transition-transform duration-75 ease-linear"
                style={{
                    width: '2000px', height: '2000px', transform:
                ...
                {activePads.map(pad => (
                    <div key={pad.id} className={`absolute rounded-xl border-b-8 border-r-8
                                                ...
                                                ${pad.shadow}`} style={{
                            left: pad.x - pad.w / 2, top: pad.y - pad.h / 2,
                ...
                {gameState.printers.map((p, idx) => {
                    ...
                    return (
                        <div key={p.id}
                            ...
                            style={{
                                left: slot.x - 60, top: slot.y - 100, width: 120, height: 160
                            }}>
                            <div
                ...
                <div className="absolute z-50 w-12 h-12 flex items-center justify-center"
                    style={{
                        left: playerRef.current.x - 24, top: playerRef.current.y - 24,
            ...
            <div className="absolute bottom-8 left-8 w-32 h-32 bg-slate-900/50 border-4 border-slate-800 rounded-full z-50 md:hidden touch-none"
                ...
                <div className="absolute w-12 h-12 bg-blue-500 rounded-full border-b-4 border-blue-700 shadow-lg pointer-events-none transition-transform duration-75"
                    style={{
                        left: '50%', top: '50%', transform: `translate(calc(-50% +
